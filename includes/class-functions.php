...
class WBTM_Plugin_Functions {
	...
	public function wbtm_seat_plan(){
	  ...
	  <table id="repeatable-fieldset-seat-one-dd" width="100%">
		...
		<p><a id="add-seat-row-dd" class="button" href="#"><?php _e('Add New Seat Row','bus-ticket-booking-with-seat-reservation'); ?></a></p>
	  </div>
	  <?php
		die();
	  }
...
// Getting all the bus stops name from a stop name
	 ...
	 if(sizeof($pos2) > 0){  unset($all_bp_stops[$pos2]); }
...
if(is_array($start_stops) && sizeof($start_stops) > 0){
	...
	$myArrayInit = $full_array; //<-- Your actual array
	  ...
	  if(sizeof($pos2) > 0){   unset($new_arr[$pos2]); }
		...
		public function wbtm_get_this_bus_seat_plan(){
			...
			public function wbtm_buffer_time_check($bp_time,$date){
				...
				public function wbtm_get_seat_status($seat,$date,$bus_id,$start,$end){
					...
					// get bus price
					public function wbtm_get_bus_price($start,$end, $array) {
							foreach ($array as $key => $val) {
				...
				public function wbtm_check_od_in_range($start_date, $end_date, $j_date){
				 ...
				 public function wbtm_get_seat_cehck_before_order($seat,$date,$bus_id,$start){
					...
					public function create_bus_passenger($order_id,$bus_id,$user_id,$start,$next_stops,$end,$b_time,$j_time,$_seats,$fare,$j_date,$add_datetime,$user_name,$user_phone,$user_gender,$user_address,$wbtm_extra_bag_qty,$usr_inf,$counter,$status ){
					...
					foreach ( $order->get_items() as $item_id => $item_values ) {
						...
						$this->create_bus_passenger($order_id,$bus_id,$user_id,$start,$next_stops,$end,$b_time,$j_time,$_seats,$fare,$j_date,$add_datetime,$user_name,$user_phone,$user_gender,$user_address,$wbtm_extra_bag_qty,$usr_inf,$counter,0 );
...
jQuery('#mage_bus_search_button').on('click', function () {
            ...
            var r_date = jQuery('#r_date').val();          
              jQuery.ajax({
                ...
                data: {"action": "wbtm_ajax_search_bus", "bus_start_route":bus_start_route, "bus_end_route":bus_end_route, "j_date":j_date, "r_date":r_date},
                    beforeSend: function(){
...
jQuery('.wbtm_next_day_search').on('click', function () {
            ...
            var r_date = jQuery(this).data('rdate');    
              jQuery.ajax({
                type: 'GET',
				url:wbtm_ajax.wbtm_ajaxurl,
                data: {"action": "wbtm_ajax_search_bus_tab", "bus_start_route":bus_start_route, "bus_end_route":bus_end_route, "j_date":j_date, "r_date":r_date},
                    beforeSend: function(){
