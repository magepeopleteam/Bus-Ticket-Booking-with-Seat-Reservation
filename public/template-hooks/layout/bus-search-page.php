            ...
            <div class="mage_flex">
                ...
                <div class="mage_bus_info flexEqual_flexCenter">
                    ...
                    <h6 class="mage_hidden_xxs"><?php echo mage_bus_type(); ?></h6>
                        <h6 class="mage_hidden_xs"><strong><?php echo wc_price($seat_price); ?></strong>/<span><?php mage_bus_label('wbtm_seat_text', __('Seat', 'bus-ticket-booking-with-seat-reservation'));?></h6>
...
function mage_bus_item_seat_details($return) {
                ...
                <div class="mage_bus_details_short">
                    ...
                    <div class="textCenter mage_bus_seat_list">
                        ...
                        <div class="mage_bus_selected_list mage_bus_sub_total padding">
                            ...
                            <h5>
                            <strong><?php mage_bus_label('wbtm_sub_total_text', __('Sub Total :', 'bus-ticket-booking-with-seat-reservation')); ?></strong><strong class="mage_bus_sub_total_price"><?php echo wc_price(0); ?></strong>
                            </h5>
    ...
    <input type="hidden" name="mage_bus_title" value="<?php  mage_bus_label('wbtm_passenger_info_seat_text', __('Passenger Info seat :', 'bus-ticket-booking-with-seat-reservation')); ?>"/>
    <?php
}
            ...
            if ($seat_plan_type > 0) {
                ...
                foreach ($seats_rows as $seat) {
                    ...
                    for ($i = 1; $i <= $seat_col; $i++) {
                        ...
                        $seat_html .= mage_bus_seat($seat_plan_type, $seat_name, $price, false, $return, 0, $all_stopages_name);
                    }
                    $seat_html .= '</div>';
                }
                echo $seat_html;
                ?>
                <script>
                    const elements = document.getElementsByClassName("defaultLoaderFixed");
                ...
                foreach ($seats_rows as $seat) {
                    ...
                    foreach ($seat_col_arr as $seat_col) {
                        ...
                        echo mage_bus_seat($seat_plan_type, $seat_name, $price, false, $return, $seat_col, $all_stopages_name);
                    }
                    echo '</div>';
                }
            } else {
        ...
        if (is_array($seats_dd) && sizeof($seats_dd) > 0) {
            ...
            foreach ($seats_dd as $seat) {
                ...
                for ($i = 1; $i <= $seat_col_dd; $i++) {
                    ...
                    echo mage_bus_seat($seat_plan_type, $seat_name, $price, true, $return, 0, $all_stopages_name);
                }
                echo '</div>';
            }
            echo '</div>';
...
//bus seat place
function mage_bus_seat($seat_plan_type, $seat_name, $price, $dd, $return, $seat_col, $all_stopages_name) {
    global $mage_bus_total_seats_availabel;
    $seat_panel_settings = get_option('wbtm_bus_settings');
    ...
    if (strtolower($seat_name) == 'door') {
        ...
        $get_booking_data = get_seat_booking_data($seat_name, $return);

        $partial_route_condition = false; // init value
        $seat_status = false; // init value
        if($get_booking_data) {
            $seat_status = $get_booking_data['status'];
            $seat_boarding_point = $get_booking_data['boarding_point'];
            $seat_droping_point = $get_booking_data['dropping_point'];
...
function mage_bus_route_title($return) {
    ...
    <div class="bgLight_mar_t_textCenter_radius_pad_xs_justifyAround mage_title">
        ...
        <h4><strong><?php echo mage_bus_isset('j_date'); ?></strong></h4>
    </div>
    <?php
}