...
?>
    <tr class="seat<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>_lists ">
      ...
      if($_seats[$text_field_name]){ ?>
              ...
              <?php } else { ?> 				
          	<a data-seat='<?php echo $_seats[$text_field_name]; ?>' id='seat<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>_<?php echo $_seats[$text_field_name]; ?>' data-sclass='Economic' class='seat<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>_blank blank_seat'>
...
<div class="bus-seat-panel-ss">
  <div style='border: 1px solid #ddd;padding: 5px;width:204px; text-align:<?php if($current_driver == 'driver_left'){ echo 'left'; }else{ echo 'right'; } ?>'>       
